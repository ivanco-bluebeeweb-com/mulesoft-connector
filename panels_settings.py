...
(disconnect per organization/environment) for MuleSoft Connector. Split out
of panels.py per the same convention as Aidentika's / n8n's / Make.com
...
here, one row per connected organization/environment. The one secondary
...
def _connection_row(c: dict) -> ui.UINode:
    ...
    return ui.Stack(direction="v", gap=1, align="start", children=[
        ...
        ui.Button(
            ...
            on_click=ui.Call("disconnect_mulesoft", {"connection_id": c.get("id")}),
        ),
    ])
...
@ext.panel("mulesoft_settings", slot="center")
async def mulesoft_settings_panel(ctx) -> ui.UINode:
