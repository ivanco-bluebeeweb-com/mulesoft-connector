...
подтвердил объём релиза 2026-08-20 — «максимум» (Ярус 1+2+3). Реализация,
деплой и прайсинг завершены (см. `PRICING_HISTORY.md` этой папки).

**Исправленный процесс-инцидент (2026-08-20):** приложение было отправлено
на `submit_for_review` ДО выставления цены — нарушение канонического
`PRICING_POLICY.md`. Влад указал на ошибку; порядок исправлен и закреплён
в `PRICING_POLICY.md` §1 навсегда: `deploy_app` → `update_pricing` →
`submit_for_review`, никогда наоборот, для ЛЮБОГО приложения на этой
платформе.
